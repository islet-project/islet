...
# tflite build
bazel build -c opt --define tflite_with_flex=true //tensorflow/lite:libtensorflowlite.so --jobs=$1
bazel build -c opt --config=monolithic tensorflow/lite/delegates/flex:tensorflowlite_flex --jobs=$1
