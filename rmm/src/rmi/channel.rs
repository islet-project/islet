...
use crate::rmi::gpt::{MARK_NONSECURE, MARK_REALM};
...
pub fn set_event_handler(mainloop: &mut Mainloop) {
    ...
    listen!(mainloop, rmi::LOCAL_CHANNEL_SETUP, |arg, _ret, rmm| {
        ...
        for cur_lc_pa in (lc_pa_start..lc_pa_end).step_by(GRANULE_SIZE) {
            ...
            if smc(MARK_REALM, &[cur_lc_pa])[0] != SMC_SUCCESS {
                return Err(Error::RmiErrorInput);
            }
    ...
    listen!(mainloop, rmi::LOCAL_CHANNEL_DESTROY, |arg, _ret, rmm| {
        let mut realm_id_list: [Option<usize>; 2] = [None; 2];
        let rd_list: [usize; 2] = [arg[0], arg[1]];
        let lc_pa_start = arg[2];
        let lc_ipa_start = arg[3];
        ...
        trace!(
            "LOCAL_CHANNEL_DESTROY: client_rd 0x{:X}, server_rd 0x{:X}, lc_pa: 0x{:X}, lc_ipa: 0x{:X}, lc_size: 0x{:X}",
            rd_list[0],
            rd_list[1],
            lc_pa_start,
            lc_ipa_start,
        ...
        trace!(
            "LOCAL_CHANNEL_DESTROY: Set local channel granule to GranuleState::Undelegated with stage 2 unmapping in both realms"
        );
        for cur_lc_pa in (lc_pa_start..lc_pa_end).step_by(GRANULE_SIZE) {
            let offset = cur_lc_pa - lc_pa_start;

            let mut lc_g = get_granule_if!(cur_lc_pa, GranuleState::Data)?;
            ...
            for realm_id in realm_id_list {
                ...
                crate::rtt::data_destroy(realm_id, lc_ipa_start + offset)?;
            }

            set_granule(&mut lc_g, GranuleState::Delegated)?;
            if smc(MARK_NONSECURE, &[cur_lc_pa])[0] != SMC_SUCCESS {
                panic!(
            ...
            rmm.page_table.map(cur_lc_pa, false);
            set_granule(&mut lc_g, GranuleState::Undelegated).map_err(|e| {
                rmm.page_table.unmap(cur_lc_pa);
                e
            })?;
            rmm.page_table.unmap(cur_lc_pa);
